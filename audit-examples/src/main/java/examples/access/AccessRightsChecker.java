...
public class AccessRightsChecker {
	...
	public AccessRightsChecker(final PermissionMap permissionMap) {
	 ...
	 * @throws AccessException thrown if there is a problem checking user
	 *                         permissions
	 */
	public boolean hasPermission(final User user, final Permission permission) throws AccessException {
		final boolean allowed = permissionMap.hasPermission(user, permission);
		if (!allowed) {
			final AuditorFacade auditorFacade = new AuditorFacade(user.getName(), "ACCESS_DENIED",
					permission.getName());
	...
	public static void main(final String[] args) throws AuditException, AccessException {
		...
		final PermissionMap pm = new PermissionMap();
		...
		// just instantiated
		final AccessRightsChecker arc = new AccessRightsChecker(pm);
