...
public class Persistor {
	 ...
	 *
	 * @return An instantiated SessionFactory, always the same (the created instance
	...
	public static void setConfiguration(final Configuration cfg, final Object lock) {
		// set the lock if not set previously
		if (Persistor.cfgLock == null && lock != null) {
			Persistor.cfgLock = lock;
		}
		if (Persistor.cfgLock != null && !Persistor.cfgLock.equals(lock) && Persistor.cfg != null) {
			throw new IllegalStateException("Configuration was already set.");
		}
		Persistor.cfg = cfg;
	}

	public static void resetConfiguration(final Object lock) {
	 ...
	 * @throws HibernateException In case the session could not be created.
	 */
	...
	protected static void update(final Object o) throws HibernateException {
		Session s = null;
		Transaction tx = null;
		try {
			s = openSession();
	...
	public static void save(final Object o) throws HibernateException {
		Session s = null;
		Transaction tx = null;
		try {
			s = openSession();
	...
	public static void delete(final Object o) throws HibernateException {
		Session s = null;
		Transaction tx = null;
		try {
			s = openSession();
	 ...
	 *
	 * @param theHibernateSession The open session to close.
	 *
	 *                            In case of a problem, this method will log an
	 *                            error but won't propagate any exception -
	 *                            therefore it may be used safely in a finally{}
	 *                            block.
	 *
	 */
	protected static void close(final Session theHibernateSession) {
		try {
			if (theHibernateSession != null) {
				theHibernateSession.close();
			}
		} catch (final Exception e) {
			getStaticLogger().error("Failed to close session " + theHibernateSession, e);
		}
	}
	 ...
	 *
	 * @param theHibernateSession The open session to rollback and close.
	 * @param tx
	 */
	protected static void close(final Session theHibernateSession, final Transaction tx) {
		try {
