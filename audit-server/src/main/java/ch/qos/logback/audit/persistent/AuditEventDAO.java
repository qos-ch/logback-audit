...
public class AuditEventDAO extends Persistor {
	...
	static public AuditEvent findById(final Long id) throws HibernateException {
		Session s = null;
		try {
			s = openSession();
	...
	static public List<AuditEvent> findAll() throws HibernateException {
		...
		try {
			s = openSession();
			final Criteria criteria = s.createCriteria(AuditEvent.class);
	...
	static public List<Object[]> findMaxSubject() throws HibernateException {
		...
		try {
			...
			final Query q = s.createQuery("select count(ae.subject), ae.subject FROM " + AE_CLASS + " AS ae "
					+ "GROUP BY ae.subject ORDER BY count(ae.subject) desc");
			final ScrollableResults sr = q.scroll();
			final List<Object[]> l = new ArrayList<>();
			int i = 0;
			while (sr.next()) {
				System.out.println(i++);
				final Object[] oa = sr.get();
				l.add(oa);
			}
	...
	static public List<Object[]> findMaxObject() throws HibernateException {
		...
		try {
			...
			// Query q = s.createQuery("select count(ae.subject), ae.subject FROM "+
			// AE_CLASS + " AS ae "+
			// "GROUP BY ae.subject ORDER BY count(ae.subject) desc");

			final Criteria crit = s.createCriteria(AuditEvent.class);
